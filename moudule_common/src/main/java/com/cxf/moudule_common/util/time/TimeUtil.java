package com.cxf.moudule_common.util.time;
...
public class TimeUtil {
    ...
    public static Date StrToDate(String str, String format) {
        SimpleDateFormat formatDateFormat = new SimpleDateFormat(format);
        Date date = null;
        try {
            date = formatDateFormat.parse(str);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return date;
     ...
     *
     * @param year  指定年份。如果指定月份数小于0，则返回-1
     * @param month 指定月份。如果指定月份数小于1或者大于12，则返回-1
     * @return
     */
     ...
     * </p>
     *
     * @param timeStr 字符串类型的日期时间，格式应该为yyyyMMddHHmmss。如果位数不足，函数将自动补齐
     * @return long 长整型的秒数
     */
     ...
     * 判断传入参数是否为日期
     *
     * @param time 字符串类型的日期，格式应该为:yyyyMMdd
     ...
     * 判断传入参数是否为时间
     *
     * @param time 字符串类型的时间，格式应该为:HHmmss
     ...
     * 判断传入参数是否为日期时间
     *
     * @param time 字符串类型的日期时间，格式应该为:yyyyMMddHHmmss
     ...
     *
     * @param time     字符串类型的日期，格式应该为yyyyMMddHHmmss，如果位数不足将函数将自动补足位数， 前提是time应该全为数字，
     ...
     * @param type     待修改的日期类型：year = 年</div> <div>
     *                 quarter = 季度</div> <div>
     *                 month = 月</div> <div>
     *                 week = 周</div> <div>
     *                 day = 日</div> <div>
     ...
     *
     * @param start 字符串类型的日期，格式应该为yyyyMMddHHmmss，如果位数不足将函数将自动补足位数， 前提是time应该全为数字，
     *              而且time的位数应该在4、6、8、10、12、14、17之中，即“年”、“年月”、“年月日”、“年月日时”、
     *              “年月日时分”、 “年月日时分秒”和“年月日时分秒毫秒”来进行处理。否则，函数将返回空。同时要求输入时不得少
     *              于要求计算的时间单位，如计算年，则输入要保证4位年份
     * @param end   同start
     ...
     */
    public static float different(String start, String end, String type) {
        long typeL = -1;
        if (type.equals("year")) {
            typeL = 1;
        } else if (type.equals("month")) {
        ...
        switch ((int) typeL) {
            ...
            case (int) 7:
                if (start.length() < 10 || end.length() < 10)
                    return -1;
                ret = Math.abs((cal1.getTimeInMillis() - cal2.getTimeInMillis())) / 1000 ;
                ret=ret/60;
                ret=ret/60;
                break;
            case (int) 8:
     ...
     * @return
     */
    public static long getTimeMonth(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        Integer cntMonth = calendar.get(Calendar.MONTH) + 1;
        return cntMonth.longValue();
    }
    ...
    public static long getTimeDay(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        Integer cntMonth = calendar.get(Calendar.DATE) - 1;
        return cntMonth.longValue();
    }
    ...
    public static int getDay(int year,int month) {
        ...
        int monthNum = calendar.getActualMaximum(Calendar.DATE);//获取指定年份中指定月份有几天
        return monthNum;
    }
     ...
     * @return
     */
    public static String getDayFormat(int year, int month, int day) {
...
//    }
    ...
    public static void main(String[] args) throws ParseException {
        ...
        System.out.println(getDayFormat(2018,5,27));
    }
    ...
    public static String dateToWeek(String datetime) {
        String newTime = datetime.substring(0,8);
        SimpleDateFormat f = new SimpleDateFormat("yyyyMMdd");
        String[] weekDays = { "周日", "周一", "周二", "周三", "周四", "周五", "周六" };
//        String[] weekDays = { "Sun", "Mon", "Tues", "Wed", "Thu", "Fri", "Sat" };

        Calendar cal = Calendar.getInstance();// 获得一个日历
        cal.set(Calendar.MONTH,-1);
        Date datet = null;
        try {
            datet = f.parse(newTime);
            cal.setTime(datet);
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    public static List<String> getWeekdays(int year, int month, int day) {
        List<String> list = new ArrayList<>();
        Calendar calendar = Calendar.getInstance();
